...
void ChooseMusic(uint8_t enable){
	static uint8_t _idx=1;
	if(idx !=_idx || enable)
	{
		_idx=idx;
		...
		switch(idx)
		{
			case 0:BuzzerPlay(TUNE, VOL_ALARM, PRIO_ALARM);break;
			case 1:BuzzerPlay(STAY, VOL_ALARM, PRIO_ALARM);break;
		}
	}
	
	switch(idx)
	{
		case 0:OledShowStr(64,14,PSTR(">SUMMER"),OLED_8X8);break;
		case 1:OledShowStr(64,14,PSTR(">STAY  "),OLED_8X8);break;
	}
}

static bool MusicSelect(){	
	BuzzerStop();
	AnimotionLeave(back); 
	return true;
}
...
static uint8_t vert[8][3];
static uint8_t x1=0, x2=0, y1=0, y2=0;

static void MusicDraw(){
	...
	//OledShowImage(0, 0, 64, 64, (const uint8_t *)OLED_GRAM,0);
	//memset(&OLED_GRAM, 0x00, FRAME_BUFFER_SIZE/2);
	OledShowStr(64, 28,PSTR("Cube "), OLED_8X8);
	OledShowStr(64, 40,PSTR("Rotation"), OLED_8X8);

	ChooseMusic(0);  //ѡ�����  
}

void MusicMenu(void){
	oldDrawFunc = SetDrawFunc(MusicDraw);
	oldBtn1Func = SetButtonSignedFunc(BTN1, MusicUp);
	oldBtn2Func = SetButtonSignedFunc(BTN2, MusicSelect);
	oldBtn3Func = SetButtonSignedFunc(BTN3, MusicDown);
	ChooseMusic(1);
	AnimotionInto(NULL);
}
